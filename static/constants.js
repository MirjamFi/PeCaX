export const layoutArray = ["dagre (default)", "klay", "breadthfirst", "cose-bilkent", "grid"];
export const basicstyle = {selector: 'node',
        style: {
          width: 50,
          height: 50,
          shape: 'ellipse',
          'background-color': 'white',
          'border-color' : 'black',
          'border-style' : 'solid',
          'border-width' : '2',
          "text-valign" : "center",
          "text-halign" : "center",
          "font-size" : 10,
          "color":"black",
          'curve-style' : 'bezier'
      }};
export const basicedgestyle = {selector: 'edge',
        style: {
          // 'target-arrow-shape': 'triangle',
          'arrow-scale' : 2,
          'curve-style' : 'bezier',
          // 'label':'data(interactionShort)',
          'font-size':16,
          'text-rotation':'autorotate',
          'font-weight':800,
          'target-arrow-shape' : 'vee'
          
        }};
export const actstyle = {selector: 'edge[interaction = \'activation\']',
          style: {
            'target-arrow-shape': 'triangle',
        }};
export const stimstyle = {selector: 'edge[interaction = \'stimulation\']',
          style: {
            'target-arrow-shape': 'triangle',
        }};
export const targetsstyle = {selector: 'edge[interaction = \'targets\']',
  style: {
    'target-arrow-shape': 'triangle',
}};
export const expstyle = {selector: 'edge[interaction = \'expression\']',
          style: {
            'target-arrow-shape': 'triangle',
        }};
export const inhistyle = {selector: 'edge[interaction = \'inhibition\']',
          style: {
            'target-arrow-shape': 'tee',
        }}
export const reprstyle = {selector: 'edge[interaction = \'repression\']',
          style: {
            'target-arrow-shape': 'tee',
        }}
export const bindstyle = {selector: 'edge[interaction = \'binding/association\']',
          style: {
            'target-arrow-shape': 'triangle-cross',
        }}
export const dissostyle = {selector: 'edge[interaction = \'dissociation\']',
          style: {
            'target-arrow-shape': 'triangle-cross',
        }}
export const compstyle = {selector: 'edge[interaction = \'compound\']',
          style: {
            'target-arrow-shape': 'circle',
        }}
export const noncovstyle = {selector: 'edge[interaction = \'non-covalent binding\']',
          style: {
            'target-arrow-shape': 'circle',
        }}
export const molinteractstyle = {selector: 'edge[interaction = \'molecular interaction\']',
          style: {
            'target-arrow-shape': 'circle',
        }}
export const indeffstyle = {selector: 'edge[interaction = \'indirect effect\']',
        style: {
          'line-style': 'dotted',
          'target-arrow-shape': 'triangle'
        }}
export const missstyle = {selector: 'edge[interaction = \'missing interaction\']',
        style: {
          'line-style': 'dashed',
          'target-arrow-shape': 'triangle'
        }}
export const statestyle = {selector: 'edge[interaction = \'state change\']',
          style: {
            'target-arrow-shape': 'square',
        }}
export const controlstyle = {selector: 'edge[interaction = \'control\']',
          style: {
            'target-arrow-shape': 'square',
        }}
export const phosphostyle = {selector: 'edge[interaction = \'phosphorylation\']',
        style: {
          'target-arrow-shape': 'diamond',
          'target-label':'+p',
          'target-text-offset':20
        }}
export const dephosphostyle = {selector: 'edge[interaction = \'dephosphorylation\']',
          style: {
            'target-arrow-shape': 'diamond',
            'target-label':'-p',
          'target-text-offset':20
        }}
export const glycostyle = {selector: 'edge[interaction = \'glycosylation\']',
          style: {
           'target-arrow-shape': 'diamond',
           'target-label':'+g',
          'target-text-offset':20
        }}      
export const ubiquistyle = {selector: 'edge[interaction = \'ubiquitination\']',
          style: {
            'target-arrow-shape': 'diamond',
            'target-label':'+u',
          'target-text-offset':20
        }}
export const methystyle = {selector: 'edge[interaction = \'methylation\']',
          style: {
            'target-arrow-shape': 'diamond',
            'target-label':'+m',
          'target-text-offset':20
        }};

export const backgroundimg_bool = 'data:image/svg+xml;base64,'
export const backgroundimg_num = 'data:image/svg+xml;base64,'