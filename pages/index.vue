  	...
  	<div v-show="showFileLoad" class="row">
	  	<div class="column" style="position:relative;">
	  		...
	  		<div v-show="showFileLoad" id='vcfbutn' style="position:relative; bottom:0px; margin-left:-150px;left:47.5%;width:100%;">
	        	<input type="file" accept=".vcf"id="vcffile" ref="vcffile" v-on:change="handleFileUpload()"/>
	        	<button class= 'butn' 
	        		v-on:click="analyzeVCF();
	        		showFileLoad = false;
		         	showStatus=true;">Submit .VCF</button>
			</div>
...
Sürün, B., Schärfe, C.P., Divine, M.R., Heinrich, J., Toussaint, N.C., Zimmermann, L., Beha, J. and Kohlbacher, O., 2020. ClinVAP: a reporting strategy from variants to therapeutic options. Bioinformatics, 36(7), pp.2316-2317. <a href="https://doi.org/10.1093/bioinformatics/btz924">https://doi.org/10.1093/bioinformatics/btz924</a></p>
		 ...
		 <div class="column">
		  	<div class="verticalLine">
				...
				<div v-show="showFileLoad" class="text-center" id="jsonbtn">
			        <input type="file" accept=".json"id="file" ref="file" v-on:change="handleFileUpload()"/>
			        <button class= 'butn' v-on:click="
			          showJSON()">Submit .JSON</button><br>
			         <input type="text" name="vcfID" placeholder="Job ID" />
			         <button class= 'butn' v-on:click="
			          showFileLoad = false;
			          showTable=true;
	...
	<div v-show="showStatus">
		...
		<p style="border:3px; border-style:solid; border-color:#BDBDBD; padding: 1em;" class="text-center">Show ClinVAP status here.</p>
		<div><button class= 'butn' v-on:click="
			          showFileLoad = false;
			          showTable=true;
    ...
    <div  v-show="showTable" width=100% ref="all">
    	...
    	<div ref = "content_mskdg" id="mskdg_content">
	        <b-button
	         ...
	         >
          		<Strong class="text-center" >Somatic Mutations in Known Driver Genes </Strong><b-icon data-html2canvas-ignore="true" class="float-right" icon="arrows-expand" v-if="visibleDrivers"></b-icon> <b-icon class="float-right" icon="arrows-collapse" v-else></b-icon>
        	</b-button>
	    ...
	    <div ref = "content_ptpda" id="ptpda_content">
	        <b-button
	          ...
	          block variant="primary"
	         >
	          <Strong>Somatic Mutations with Known Pharmacogenetic Effect</Strong><b-icon data-html2canvas-ignore="true" class="float-right"icon="arrows-expand" v-if="visiblePharma"></b-icon> <b-icon class="float-right" icon="arrows-collapse" v-else></b-icon>
	        </b-button>
	    ...
	    <div ref="content_smptag">
	        ...
	        <b-collapse id="collapse-4" v-model="visibleAffect" class="mt-2">
	          ...
	          <div ref = "content_ptpia" id="ptpia_content">
	            ...
	            <b-button
	              ...
	              block variant="primary"
	              >
	              <Strong>CIViC Summary of Drugs Targeting Affected Genes</Strong><b-icon data-html2canvas-ignore="true" class="float-right"icon="arrows-expand" v-if="visibleAffectCivic"></b-icon> <b-icon class="float-right" icon="arrows-collapse" v-else></b-icon>
	            </b-button>
	        ...
	        <p></p>
	          ...
	          <b-button
	            ...
	            block variant="primary"
	           >
	            <Strong>Summary of Cancer Drugs Targeting Affected Genes</Strong><b-icon class="float-right"icon="arrows-expand" v-if="visibleAffectCancer"></b-icon> <b-icon class="float-right" icon="arrows-collapse" v-else></b-icon>
	          </b-button>
...
<script>
  ...
  export default {
    ...
    methods: {
	    ...
	    getGraphFromGenes(gene){
			var gene = 'BRAF';
	    	console.log(gene)
	    	var graphml = this.getGraphforGene(gene)
		...
		getGraphforGene(name){

		  const UrlNetworkInventory = '/sbml4j/networkInventory';
		  ...
		  return axios.get(UrlNetworkInventory, ParamNetworkInventory)
		    .then(data => {
		      const networkInventory = data.data
		      for(let entry of networkInventory){
		        if(entry['networkMappingType'] == 'REGULATORY'){
		          const UrlListGenes = '/sbml4j/networkInventory/'+entry['entityUUID']+'/filterOptions'
		          ...
		          return axios.get(UrlListGenes, ParamListGenes)
		            .then(data => {
		              var listofGenes = data.data.nodeSymbols;
		              if(listofGenes.includes(name)){
		                const UrlNetwork = '/sbml4j/context?baseNetworkUUID='+entry['entityUUID']+'&gene='+name+'&minSize=1&maxSize=1&format=graphml'
		                const ParamNetwork = {
		                  headers:{
		                    'user':'openMTB'
		                  },
		                ...
		                return axios.get(UrlNetwork, ParamNetwork)
	    ...
	    showJSON(){
	        var file = this.$refs.file.files[0]; // FileList object
	        if(!file){
	        ...
	        this.getGraphFromGenes('BRAF')
	    },
	    analyzeVCF(){
	    	var file = this.$refs.vcffile.files[0]
	    	var formData = new FormData();
	    	formData.append("vcf", file);
			axios.post('http://localhost/upload-input', formData, {
			    headers: {
	    ...
	    download_DriverNetwork(){
	    	var graphml = this.getGraphforGene('BRAF')
	    		.then(response => graphml = response)
	    	graphml.then(function(response){
				if(response == undefined){
					alert("No network found for "+gene)
	  ...
	  //   	var formData = new FormData();
	  //   	formData.append("vcf", file);
			// axios.post('http://localhost/uresults/'+file, formData, {
			//     headers: {
			//       'Content-Type': 'multipart/form-data'
			//     }})
			// .then(function (response) {
